# Dungeon Dine & Dash: Astera

![Title Screen](https://media.discordapp.net/attachments/1467032167543476246/1467033579694329876/image.png?ex=697ee8fd&is=697d977d&hm=83c6b452232f67f7872870205769b09a0085b0bdda741d2ad2c5b3534201e5c6&=&format=webp&quality=lossless&width=1275&height=559)

* **[ITCH.IO](https://stevnath.itch.io/dungeon-dine-dash-astera)**
* **[Web Game Version](https://stevnath.github.io/Dungeon-Dine-And-Dash-Astera-Web-Version/)**

---
**Dungeon Dine & Dash: Astera** adalah permainan *Role-Playing Game* (RPG) berbasis giliran yang memadukan elemen eksplorasi *dungeon* klasik dengan simulasi manajemen ekonomi. Pemain akan mengendalikan karakter utama yang terjebak di dunia asing, di mana ia harus berburu monster, mengolah daging hasil buruan menjadi hidangan kuliner, dan membangun kekuatan tim untuk menaklukkan tantangan terbesar di Astera.

### Game Engine
Permainan ini dikembangkan menggunakan:
* **RPG Maker MZ**

### Tim Pengembang (Credits)
Dibuat dengan penuh dedikasi oleh:
* **Steven Nathanael** — [@StevNath](https://github.com/StevNath)
* **Putra Parlindungan** — [@PutraParlindungan](https://github.com/PutraParlindungan)
* **M. Nurulloh** — [@Nurulloh21](https://github.com/Nurulloh21)
* **M. Hafidz Putra** — [@HFDZ-PTR](https://github.com/HFDZ-PTR)
* **Joshua Natanael** — [@JoshuaImanuel](https://github.com/JoshuaImanuel)


---
© 2026 Dungeon Dine & Dash: Astera Team.


---

## 1. Inisialisasi dan Pemilihan Karakter

![Character Selection](https://media.discordapp.net/attachments/1467032167543476246/1467033429760544840/Screenshot_2026-01-31_120357.png?ex=697ee8d9&is=697d9759&hm=6467fab586a88980c041b3271bc5c3d917ee4b536b7d2ca0ffabab4fd74e5c84&=&format=webp&quality=lossless&width=988&height=750)

Permainan dimulai dengan adegan karakter utama yang terbangun di sebuah aula misterius. Sebelum memulai penjelajahan, pemain diwajibkan untuk memilih satu dari lima identitas tempur. Pemilihan ini akan menentukan statistik dasar dan gaya bermain utama karakter:

* **Zephyr (Magic Swordsman):** Petarung hibrida yang menyeimbangkan serangan fisik dan magis (HP 544, MP 117).
![Character Selection](https://media.discordapp.net/attachments/1467032167543476246/1467034092439732254/Screenshot_2026-01-31_122558.png?ex=697ee977&is=697d97f7&hm=309ceaf38f7ea5f66b4498961eefb7c3b9ab458083f04754db982fadb4f40466&=&format=webp&quality=lossless&width=988&height=750)

* **Ragul (Martial Artist):** Berfokus pada ketahanan fisik dan seni bela diri dengan HP tertinggi (639).
![Character Selection](https://media.discordapp.net/attachments/1467032167543476246/1467034092762828961/Screenshot_2026-01-31_122623.png?ex=697ee977&is=697d97f7&hm=250684f06a6794512a4a09608df104a08269fca3ab6ec474168a5c81362e0bce&=&format=webp&quality=lossless&width=981&height=750)

* **Perceval (Sorcerer):** Spesialis sihir murni dengan serangan magis tertinggi (MAT 29) namun pertahanan rendah.
![Character Selection](https://media.discordapp.net/attachments/1467032167543476246/1467034093299568772/Screenshot_2026-01-31_122635.png?ex=697ee978&is=697d97f8&hm=410240e897f8043e738a9d91a1139b633306632b064370d0511c83705b37341e&=&format=webp&quality=lossless&width=981&height=750)

* **Lynae (Knight):** Kelas pelindung (Tank) dengan pertahanan fisik yang kokoh.
![Character Selection](https://media.discordapp.net/attachments/1467032167543476246/1467034093710737550/Screenshot_2026-01-31_122647.png?ex=697ee978&is=697d97f8&hm=08f4922ebfae42d18517a293883c9f0a042df0d4c64bf2d1b32ac0d84e495234&=&format=webp&quality=lossless&width=989&height=750)

* **Arlena (Bandit):** Mengandalkan kecepatan (*Agility*) dan keberuntungan dalam *looting*.
![Character Selection](https://media.discordapp.net/attachments/1467032167543476246/1467034094117322792/Screenshot_2026-01-31_122659.png?ex=697ee978&is=697d97f8&hm=d60d82e27e50417ced8548b999c4df7c30bdc4d62e87d255d40f2280cbc7ef7b&=&format=webp&quality=lossless&width=976&height=750)

---

## 2. Navigasi Dunia dan Peta

Setelah memilih karakter, pemain akan berada di titik persimpangan strategis (*Spawn Map*) seperti terlihat pada gambar.
![Spawn Area Navigation](https://media.discordapp.net/attachments/1467032167543476246/1467035450626801734/image.png?ex=697eeabb&is=697d993b&hm=f680bbcc0fe203a30caa203425914dc1c2cbcd2378cbb9ad4047bf4ced879c9b&=&format=webp&quality=lossless&width=1025&height=776)

* **Ke Arah Kiri (Stage Hutan):** Menuju area hutan untuk berlatih melawan musuh lemah dan mengumpulkan modal awal.
![Spawn Area Navigation](https://media.discordapp.net/attachments/1467032167543476246/1467035067359694909/Screenshot_2026-01-31_120549.png?ex=697eea60&is=697d98e0&hm=9e7ff00c2ddd326cf41c352302e1504249511ab864473252ce07daa144eb874f&=&format=webp&quality=lossless&width=986&height=750)
![Spawn Area Navigation](https://media.discordapp.net/attachments/1467032167543476246/1467042722354434091/Screenshot_2026-01-31_121342.png?ex=697ef181&is=697da001&hm=336177915469051bce0cf3ef9cd1562395381cbe037e5c67c4338ab77a77a25f&=&format=webp&quality=lossless&width=985&height=750)

* **Ke Arah Kanan (Kota Astera):** Menuju gerbang kota utama untuk mengakses toko, Dungeon Obelix dan fasilitas lainnya.
![Spawn Area Navigation](https://media.discordapp.net/attachments/1467032167543476246/1467035067002917038/Screenshot_2026-01-31_120515.png?ex=697eea60&is=697d98e0&hm=c2dd57def86314817ab0cf0656aded3839515395f762de21dff4931bd23bab52&=&format=webp&quality=lossless&width=989&height=750)
![Spawn Area Navigation](https://media.discordapp.net/attachments/1467032167543476246/1467042722677264567/Screenshot_2026-01-31_121524.png?ex=697ef181&is=697da001&hm=e7393b4be2881af3ecc0b28e972ac01199b332307cbfbbde8ea145e43938d758&=&format=webp&quality=lossless&width=981&height=750)


---

## 3. Fasilitas Kota dan Pusat Ekonomi

![Town Center](https://media.discordapp.net/attachments/1467032167543476246/1467035753581121647/Screenshot_2026-01-31_120623.png?ex=697eeb03&is=697d9983&hm=c89b52d444ec9a0a3529372ae4c928966c2a92b9b34a295f098e4d551f519698&=&format=webp&quality=lossless&width=985&height=750)

**Pusat Kota (Town Center)**
Kota Astera adalah hub utama pemain. Di tengah kota, terdapat monumen batu besar bernama **Obelix** (lihat gambar di atas). Pemain dapat berinteraksi dengan Obelix menggunakan tombol **Enter** untuk membuka portal menuju *dungeon* tingkat lanjut.

### General Shop (Toko Umum)

![Shop Interior](https://media.discordapp.net/attachments/1467032167543476246/1467035753900150814/Screenshot_2026-01-31_120742.png?ex=697eeb03&is=697d9983&hm=b5fe468b29ab84519ad70c84bdecb0f384c28db53b11bed8102437c2f2738bc3&=&format=webp&quality=lossless&width=989&height=750)

Gambar di atas menunjukkan interior dari General Shop. Ini adalah pusat aktivitas ekonomi di mana pemain dapat melakukan:
1.  **Jual Beli:** Menukar *loot* menjadi Gold.
2.  **Resep:** Mempelajari menu baru melalui buku di atas meja.
3.  **Memasak:** Menggunakan tungku di pojok kanan atas.

![Shop Menu](https://media.discordapp.net/attachments/1467032167543476246/1467036806871519367/Screenshot_2026-01-31_122026.png?ex=697eebfe&is=697d9a7e&hm=0451f6833617e06738161c4997c4ea614183a29ae0d21170f3611c555e8fe980&=&format=webp&quality=lossless&width=979&height=750)

**Sistem Memasak & Transaksi**
Pemain wajib mengolah bahan mentah menjadi masakan (seperti *Roasted Meat* pada gambar) untuk meningkatkan nilai jualnya. Daging didapatkan dari monster, kemudian dimasak di toko, dan dijual kembali untuk keuntungan maksimal.

### Fasilitas Penunjang Lainnya

Selain toko, pemain juga dapat mengunjungi:
* **Blacksmith:** Tempat membeli senjata baru dengan berinteraksi pada lingkaran sihir di dekat etalase.
![Blacksmith or Tavern](https://media.discordapp.net/attachments/1467032167543476246/1467035754575171697/Screenshot_2026-01-31_121200.png?ex=697eeb04&is=697d9984&hm=274594cf82208f6e781471a279dba5c06cc15201ca3db8f688f1b7e2d5ad8cc9&=&format=webp&quality=lossless&width=985&height=750)

* **Tavern:** Hub sosial untuk merekrut anggota tim (*party*) tambahan.
![Blacksmith or Tavern](https://media.discordapp.net/attachments/1467032167543476246/1467035754260857047/Screenshot_2026-01-31_121012.png?ex=697eeb04&is=697d9984&hm=e8e98eca7ad9604f99dbd15c00989954b9c64122a054c391e2b1d4b7959b346e&=&format=webp&quality=lossless&width=976&height=750)

---

## 4. Sistem Pertarungan (Battle System)

![Battle System](https://media.discordapp.net/attachments/1467032167543476246/1467036994033942802/Screenshot_2026-01-31_121742.png?ex=697eec2b&is=697d9aab&hm=03a4bc63d5ce26df2ab585f3223b4568302b2546e3cd18872718061bcf1fd8e3&=&format=webp&quality=lossless&width=1025&height=771)

Mekanisme pertempuran menggunakan sistem *Turn-Based RPG*. Ketika pemain berpapasan dengan musuh, layar akan beralih ke mode pertempuran seperti gambar di atas.

Pemain memiliki opsi taktis:
* **Attack:** Serangan fisik dasar.
* **Magic:** Serangan elemen (menggunakan MP).
* **Special:** Jurus khusus karakter.
* **Guard:** Bertahan untuk mengurangi kerusakan.

**Hadiah Kemenangan (Victory Rewards):**
Setelah berhasil mengalahkan musuh, pemain akan mendapatkan **EXP** (Level Up), **Gold** (Uang), dan **Daging Monster** (Drop Item) yang krusial untuk dimasak.

---

## 5. Kontrol Permainan
Interaksi permainan dirancang sederhana menggunakan papan ketik:
* **Navigasi:** Tombol Panah atau WASD.
* **Interaksi Utama:** Tombol **Enter** (Memilih peran, Bicara, Masak, Beli/Jual, Masuk Dungeon).
* **Menu/Batal:** Tombol Esc atau X.
